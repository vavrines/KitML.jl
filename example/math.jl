...
function ComputeSphericalBasisKarth(
    quadpts::Matrix{Float64},
    polyDegree::Int64,
    spatialDim::Int64,
)
    ...
    if spatialDim == 3
        for idx_quad = 1:(size(quadpts)[1])
            monomialBasis[:, idx_quad] = ComputeShpericalBasis3DPtKarth(
                quadpts[idx_quad, 1],
                quadpts[idx_quad, 2],
                quadpts[idx_quad, 3],
                polyDegree,
    ...
    elseif spatialDim == 1
        for idx_quad = 1:(size(quadpts)[1])
            monomialBasis[:, idx_quad] =
                ComputeShpericalBasis1DPtKarth(quadpts[idx_quad, 1], polyDegree)
        end
    else
...
function ComputeShpericalBasis3DPtKarth(
    pointX::Float64,
    pointY::Float64,
    pointZ::Float64,
    polyDegree::Int64,
...
function GetCurrDegreeSize(currDegree, spatialDim)
    return factorial(currDegree + spatialDim - 1) /
