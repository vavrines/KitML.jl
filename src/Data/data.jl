...
function regime_data_3d(cons, swx, swy, swz, f, u, v, w, K::Real, Kn::Real, μ = ref_vhs_vis(Kn, 1.0, 0.5), ω = 0.81)
    ...
    swt = -prim[1] .* (moments_conserve_slope(a, Mu, Mv, Mw, 1, 0, 0) .+ moments_conserve_slope(b, Mu, Mv, Mw, 0, 1, 0) .+ moments_conserve_slope(c, Mu, Mv, Mw, 0, 0, 1))
    A = pdf_slope(prim, swt, K)
    tau = vhs_collision_time(prim, μ, ω)
...
regime_data_3d(ks::SolverSet, w, swx, swy, swz, f) = regime_data(w, swx, swy, swz, f, ks.vs.u, ks.vs.v, ks.vs.w, ks.gas.K, ks.gas.Kn)
